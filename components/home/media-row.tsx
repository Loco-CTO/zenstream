...
export function MediaRow({ title, items, variant, stackEpisodes = false, session, viewAllHref }: { title: string; items: JellyfinItem[]; variant: "wide" | "poster"; stackEpisodes?: boolean; session?: AuthSession; viewAllHref?: string }) {
  const { t } = useI18n();
  ...
  return (
    <section className="group select-none px-4 py-4 sm:px-6 sm:py-5 md:px-10">
      <div className="mb-4 flex items-center justify-between">
        ...
        {viewAllHref ? (
          <Link href={viewAllHref} className="flex items-center gap-1 text-xs font-medium uppercase tracking-wider text-white/25 transition hover:text-white/70">
