...
function renderRow() {
  ...
  const scroller = screen.getByText("First title").closest("article")?.parentElement;
...
describe("MediaRow scrolling", () => {
  it("only shows navigation buttons when more content exists in that direction", () => {
    ...
    expect(screen.getByRole("button", { name: "Scroll Popular right" })).toBeInTheDocument();
  ...
  it("smoothly continues from pending drag movement when an arrow key takes over", () => {
    ...
    fireEvent(cardLink, pointerEvent("pointerdown", {
      button: 0,
      clientX: 200,
      pointerId: 7,
      pointerType: "mouse",
    }));
    fireEvent(scroller, pointerEvent("pointermove", {
      clientX: 150,
      pointerId: 7,
      pointerType: "mouse",
    }));
  ...
  it("eases pointer dragging and prevents the click after a drag", () => {
    ...
    fireEvent(cardLink, pointerEvent("pointerdown", { button: 0, clientX: 200, pointerId: 7, pointerType: "mouse" }));
    fireEvent(scroller, pointerEvent("pointermove", { clientX: 150, pointerId: 7, pointerType: "mouse" }));
    ...
    fireEvent(scroller, pointerEvent("pointerup", { pointerId: 7, pointerType: "mouse" }));
  ...
  it("does not capture a card click when the pointer has not dragged", () => {
    ...
    fireEvent(cardLink, pointerEvent("pointerup", {
      clientX: 200,
      pointerId: 7,
      pointerType: "mouse",
    }));
