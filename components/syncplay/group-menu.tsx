...
export function SyncplayGroupMenu({ userId, buttonClassName, playerContext = false }: { userId: string; buttonClassName?: string; playerContext?: boolean }) {
	...
	return <div className="relative" data-player-context={playerContext || undefined}>
		...
		{open && <div data-player-context={playerContext || undefined} className="absolute right-0 top-full z-[90] mt-2 w-80 rounded-xl border border-white/10 bg-black/25 p-3 shadow-2xl shadow-black/40 backdrop-blur-xl">
			...
			{groups.length === 0 ? <p className="px-2 py-4 text-xs text-white/45">{t("noSyncplayGroups")}</p> : groups.map((group) => <div key={group.id} className="flex items-center gap-2 rounded-lg px-2 py-2 hover:bg-white/5"><div className="min-w-0 flex-1"><p className="truncate text-xs text-white/85">{group.name}</p><p className="truncate text-xs text-white/45">{group.itemId ? t("syncplayWatching") : t("syncplayNoMedia")} · {group.members.length}</p></div><button onClick={() => void (active?.id === group.id ? leave() : join(group.id)).catch(() => undefined)} className="rounded-lg px-2 py-1.5 text-xs text-violet-200 hover:bg-white/10">{active?.id === group.id ? t("leaveGroup") : t("joinView")}</button></div>)}
