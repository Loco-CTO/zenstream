...
describe("SettingsPage", () => {
	...
	it("renders the design sections and changes app language", async () => {
		...
		expect(
			screen.getByRole("region", { name: "Playback" }),
		).toBeInTheDocument();
	...
	it("supports toggles, logout, and locale save errors", async () => {
		...
		render(
			<SettingsPage
				...
				userId="user-1"
				locale="en"
				onLocaleChange={vi.fn().mockRejectedValue(new Error("offline"))}
				onLogout={onLogout}
			/>,
		);
	...
	it("uses an in-app subtitle color popover instead of a native color input", async () => {
		...
		expect(
			screen.getByRole("dialog", { name: "Subtitle font color" }),
		).toBeInTheDocument();
	...
	it("separates subtitle background opacity from the following playback setting", () => {
		...
		expect(
			screen
				.getByLabelText("Subtitle background opacity")
	...
	it("localizes every settings section and control", () => {
		...
		expect(
			screen.getByRole("region", { name: "プライバシーとデータ" }),
		).toBeInTheDocument();
		expect(
			screen.getByRole("switch", { name: "次のエピソードを自動再生" }),
		).toBeInTheDocument();
