...
export function MobileNav() {
	...
	return (
		<nav className="fixed bottom-0 left-0 right-0 z-50 flex h-[calc(4rem+env(safe-area-inset-bottom))] items-center justify-around border-t border-white/5 bg-black/65 pb-[env(safe-area-inset-bottom)] backdrop-blur-2xl md:hidden">
			<Link
				href="/"
				className={`flex min-w-0 flex-1 flex-col items-center gap-1.5 px-1 py-2 ${pathname === "/" ? "text-violet-400" : "text-white/30"}`}
			>
			...
			<Link
				href="/library"
				className={`flex min-w-0 flex-1 flex-col items-center gap-1.5 px-1 py-2 ${pathname === "/library" ? "text-violet-400" : "text-white/30"}`}
			>
