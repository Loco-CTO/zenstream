	...
	return (
		<main className="min-h-screen px-4 pb-32 pt-24 sm:px-8 md:px-12 md:pb-24 md:pt-28">
			<div className="mx-auto max-w-[1500px]">
				<Link
					href="/library"
					className="mb-10 inline-flex items-center gap-2 text-xs uppercase tracking-[0.15em] text-white/35 transition hover:text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-violet-300"
				>
				...
				<header className="mb-10 border-b border-white/[0.08] pb-7">
					...
					<p className="mt-3 text-sm text-white/40">
						{albums.length} {t("albums").toLocaleLowerCase()}
					</p>
				</header>
				{albums.length === 0 ? (
					<div className="rounded-xl border border-white/10 bg-white/[0.025] px-6 py-20 text-center text-sm text-white/45">
						{t("audioEmpty")}
					</div>
				) : (
					<section aria-labelledby="artist-albums-heading">
						<h2 id="artist-albums-heading" className="sr-only">
							{t("albums")}
						</h2>
