...
const triggerClassName =
	"flex min-w-32 items-center justify-between gap-3 rounded-lg border border-[var(--c-glass-border)] bg-[var(--c-glass)] px-3 py-2 text-left text-xs text-white/75 shadow-lg shadow-black/20 outline-none backdrop-blur-xl transition hover:border-white/20 hover:bg-white/[.08] focus-visible:border-white/35 focus-visible:ring-2 focus-visible:ring-white/10";
const panelClassName =
	"fixed z-[300] max-w-[calc(100vw-1.5rem)] overflow-y-auto rounded-lg border border-[var(--c-glass-border)] bg-black/25 p-1.5 shadow-2xl shadow-black/50 backdrop-blur-xl";
	...
	useEffect(() => {
		...
		const positionMenu = () => {
			...
			const width = Math.min(
				...
				Math.max(144, window.innerWidth - edge * 2),
			);
			...
			const opensBelow =
				rect.bottom + 6 + height <= window.innerHeight - edge ||
				rect.top <= height + 6 + edge;
			setMenuStyle({
				left,
